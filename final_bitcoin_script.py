...
def simulate_bitcoin_prices(days=60, initial_price=50000, volatility=0.04, drift=0.001):
    ...
    # However, to guarantee a Golden Cross and show trades for the prompt,
    # let's try injecting a specific seed 123 if we want deterministic output,
    # or just keep it random. The user wants to simulate 60 days of data and use Golden Cross algorithm.
    ...
    # Generate dates - using timezone.utc
...
def run_trading_algorithm(df):
    ...
    # We can also just skip checking if i >= 30
    ...
    for i in range(len(df)):
        ...
        # Instead of valid_mask[i] and valid_mask[i-1], we use index tracking i >= 30 for performance
        if i >= 30:
            prev_ma7 = ma7s[i-1]
            prev_ma30 = ma30s[i-1]
