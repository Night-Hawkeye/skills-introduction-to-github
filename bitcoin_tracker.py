...
def simulate_bitcoin_prices(days=60, initial_price=50000.0, volatility=0.04, drift=0.001):
    ...
    # Generate dates using pd.date_range and timezone-aware datetime for efficiency and security best practices
...
def run_trading_algorithm(df):
    ...
    for i in range(len(df)):
        ...
        if i >= 30: # Deterministic tracking instead of pd.isna calls
            prev_ma7 = ma7s[i-1]
            prev_ma30 = ma30s[i-1]
