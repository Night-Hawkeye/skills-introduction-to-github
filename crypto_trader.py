...
def simulate_bitcoin_prices(days=60, initial_price=50000.0, volatility=0.04, drift=0.001):
    ...
    # Generate dates using pd.date_range for optimization
...
def run_trading_algorithm(df):
    ...
    # Pre-extract columns into NumPy arrays for performance
    ...
    for i in range(len(df)):
        ...
        # Deterministic check to avoid NaN moving averages
        if i >= 30:
            prev_ma7 = ma7s[i-1]
            prev_ma30 = ma30s[i-1]
