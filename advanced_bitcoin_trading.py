...
def simulate_bitcoin_prices(days=60, initial_price=50000.0, volatility=0.04, drift=0.001):
    ...
    # Generate dates using timezone-aware datetime and pd.date_range
    start_date = datetime.now(timezone.utc) - timedelta(days=days - 1)
    # Using pd.date_range instead of list comprehension for performance
...
def run_trading_algorithm(df):
    ...
    # Format dates outside the loop and use .values for performant loop over DataFrame
    ...
    # However, since we track index `i >= 30` deterministically, we can avoid pd.isna() inside loop
    ...
    for i in range(len(df)):
        ...
        # Deterministic index tracking (`i >= 30`) avoids checking pd.isna()
        if i >= 30:
            prev_ma7 = ma7s[i-1]
            prev_ma30 = ma30s[i-1]
